...
public class QuestionsController (QuestionDbContext db, IMessageBus bus): ControllerBase
{
    [Authorize]
    [HttpPost]
    public async Task<ActionResult<Question>> CreateQuestion(CreateQuestionDto dto, TagService tagService)
    {
    ...
    [HttpGet]
    public async Task<ActionResult<List<Question>>> GetAllQuestions(string? tag)
    {
    ...
    [Authorize]
    [HttpPut("{id}")]
    public async Task<ActionResult> UpdateQuestion(string id, CreateQuestionDto dto, TagService tagService)
    {
        var question = await db.Questions.FindAsync(id);
        if (question is null) return NotFound();
